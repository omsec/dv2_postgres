with ts as (
	select
		...
		coalesce(nullif(trim(both from t.hk_address::character varying), ''::text), '^^'::text),
		coalesce(nullif(trim(both from t.load_ts::character varying), ''::text), '^^'::text)
		)), 'hex'::text) as hk_address_snapshot,*/
		t.hk_address,
		t.adr_rowid,
		t.rowhash,
		t.adr_created_at,
		...
		lead(t.usr_deleted_by_login_name) over(partition by t.hk_address order by t.load_ts) as usr_deleted_by_login_name,
		t.cod_type,
		t.txt_type_en,
		t.adr_priority,
		t.adr_address,
		...
		coalesce(
			...
			lead(t.load_ts) over(partition by t.hk_address order by t.load_ts) - interval '1 ms',
			to_timestamp('2099-12-31 23:59:59.999', 'yyyy-mm-dd hh24:mi:ss.fff')
		) as loadend_ts
	from {{ ref('fd_address') }} t
)
select *
from ts
where
